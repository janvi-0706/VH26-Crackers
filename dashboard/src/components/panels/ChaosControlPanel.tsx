      ...
      <div className="flex h-full items-center justify-center gap-6 px-6">
        <div className="flex flex-1 flex-col items-center gap-2.5">
          <button
            ...
            onClick={runKill}
            disabled={busy !== null}
            className="w-full rounded-xl bg-bad px-6 py-5 text-lg font-bold uppercase tracking-wide text-white
          ...
          <span className="text-center text-xs text-ink-muted">{killStatus}</span>
        </div>
        <div className="flex flex-1 flex-col items-center gap-2.5">
          <button
            type="button"
            onClick={runFlood}
            disabled={busy !== null}
            className="w-full rounded-xl bg-warn px-6 py-5 text-lg font-bold uppercase tracking-wide text-surface
              shadow-lg shadow-warn/40 transition-transform hover:scale-[1.03] hover:shadow-warn/60
              active:scale-[0.98] disabled:opacity-60 disabled:hover:scale-100"
          >
            {busy === "flood" ? "Flooding…" : "🌊 Duplicate flood"}
          </button>
          <span className="text-center text-xs text-ink-muted">{floodStatus}</span>
        </div>
      </div>
    </Panel>
  );
