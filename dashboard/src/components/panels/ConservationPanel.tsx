...
export function ConservationPanel({ latest }: { latest: MetricsFrame | null }) {
  ...
  return (
    ...
    >
      <div
        className={`flex h-full min-h-0 flex-col items-center justify-center gap-3 overflow-hidden rounded-lg transition-colors ${
          broken ? "bg-bad/10" : ""
        }`}
      >
        <div className="flex min-h-0 items-center gap-3">
          <span className={`font-mono text-4xl font-bold leading-none ${statusColor}`}>
            {!hasData ? "—" : broken ? "✕" : "✓"}
          </span>
        ...
        <div className="flex flex-wrap items-center justify-center gap-x-1.5 gap-y-1 px-4 font-mono text-[11px] text-ink-muted">
